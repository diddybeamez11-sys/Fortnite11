...
object ToolboxStyleLauncher {
    ...
    fun launch(activity: Activity, targetPackage: String = MINECRAFT_PACKAGE): Boolean {
        val info = try {
            ...
            ErrorLog.record(activity, "Toolbox-style launch failed: Minecraft package not installed")
            return false
        }
        ...
        if (targetPackage == MINECRAFT_PACKAGE && info.versionName != TARGET_VERSION) {
            ...
            ErrorLog.record(activity, "Toolbox-style launch rejected: expected $TARGET_VERSION, found ${info.versionName}")
            return false
        }
        ...
        val launchIntent = activity.packageManager.getLaunchIntentForPackage(targetPackage)
            ?: run {
                ...
                ErrorLog.record(activity, "Toolbox-style launch failed: no launch intent for $targetPackage")
                return false
            }
        ...
        runCatching { OverlayService.start(activity) }
            .onFailure { ErrorLog.record(activity, "Overlay startup before Minecraft launch failed", it) }
        ...
        return runCatching {
            ...
            ErrorLog.record(activity, "Started Toolbox-style Minecraft task: ${info.versionName}")
            true
        }.getOrElse {
            Log.e(TAG, "Controlled Minecraft launch failed", it)
            ErrorLog.record(activity, "Controlled Minecraft launch failed", it)
